    ...
    public class SubscriptionsService : ISubscriptionsService
    {
        ...
        public async Task<SubscriptionsDto> GetAsync()
        {
            ...
            var committees = await _db.Groups
                .AsNoTracking()
                .OrderBy(g => g.Name)
                .Select(g => new { g.Id, g.Name, g.IsPro, Until = g.ProValidUntil })
                .ToListAsync();

            var suppliers = await _db.Vendors
                .AsNoTracking()
                .OrderBy(v => v.Name)
                .Select(v => new { v.Id, v.Name, v.IsPro, Until = v.ProValidUntil })
                .ToListAsync();

            var result = new SubscriptionsDto
            {
                Committees = committees
                    .Select(c => ToRow(c.Id, c.Name, c.IsPro, c.Until, today))
                    .ToList(),
                Suppliers = suppliers
                    .Select(s => ToRow(s.Id, s.Name, s.IsPro, s.Until, today))
                    .ToList(),
            };
        ...
        */
        private static SubscriptionRowDto ToRow(
            int id, string name, bool isPro, DateTime? until, DateTime today)
        {
