...
test("בקשת תשלום בוואטסאפ: פותחת חלון, בחירת התלמידים מציגה שליחה לכל הורה", async () => {
  mockServer([dana, noam]);
  renderPage();
  await screen.findByText(/דנה כהן/);

  // לוחצים על הכפתור בכותרת → נפתח חלון עם בחירת תלמידים
  ...
  expect(sendLinks.length).toBeGreaterThanOrEqual(2);
});
