  ...
  async function readFile(file, pwd) {
    ...
    try {
      ...
      if (err?.code === "ENCRYPTED") {
        ...
        setReadError(
          isDocLike
            ? "לא הצלחנו לקרוא את הקובץ הזה 😕 קובצי Word/PDF מסוימים לא נקראים ישירות. הכי בטוח: פותחים את הקובץ, מסמנים את טבלת התלמידים, מעתיקים ל-Google Sheets (או Excel), ושומרים/מורידים כ-Excel (‏.xlsx) — ואז מעלים כאן. הקובץ שירד ייקרא מצוין (המערכת מזהה לבד שם/ת\"ז/תאריך לידה/הורים)."
  ...
  return (
    <div className="students-import">
      <p className="students-import__intro">
        ...
        העמודות (שם, יום וחודש לידה ופרטי שני ההורים) לפי שמן.{" "}
