...
*/
/*
  האם לחסום את המשתמשת בגלל תוקף מנוי.

  **החלטת בעלת המוצר (22.08.2026): לא חוסמים.** בתום חודש הניסיון המשתמשת
  עוברת ל**מסלול החינמי** — נכנסת כרגיל, רואה את כל מה שיצרה, ורק פיצ'רי
  הפרו ננעלים (ProGate בלקוח, 402 בשרת). חסימה הייתה מנתקת אותה מהנתונים
  שלה עצמה, וזה בדיוק ההפך מהמודל.

  הפונקציה נשמרת כנקודה אחת שאפשר להחזיר בה חסימה בעתיד (למשל אי-תשלום של
  מנוי שכבר נרכש), במקום לפזר את ההחלטה בין App.js למסך הכניסה.
*/
...
/*
  ייצוא כל המידע השמור על המשתמשת ועל הגנים שלה ("זכות העיון"). מחזיר
  אובייקט שהמסך הופך לקובץ להורדה. לא כולל סיסמאות או טוקני גישה.
*/
export async function exportMyData() {
  return api.get("/api/account/export");
}
