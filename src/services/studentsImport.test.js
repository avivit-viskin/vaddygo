...
test("קובץ משרד החינוך: כל השדות נשלפים, וההורה הראשי הוא האמא (הורה ב' בקובץ)", () => {
  // בקובץ משרד החינוך האמא היא "הורה ב'". בעלת המוצר ביקשה שהמספר הראשי
  // באפליקציה יהיה של האמא — לכן "הורה ב'" מהקובץ הופך ל"הורה א'" באפליקציה,
  // ו"הורה א'" מהקובץ (האב) עובר להיות "הורה ב'".
  const grid = [
    MINISTRY_HEADERS,
    [
      "הילי",
      ...
      "5",
      "3",
      "אבי", // הורה א' בקובץ = האב
      "לוי",
      "052-7654321",
      "avi@example.com",
      "דנה", // הורה ב' בקובץ = האמא
      "לוי",
      "050-1234567",
      "dana@example.com",
      "כן",
  ...
  expect(rows[0]).toMatchObject({
    ...
    // האב (הורה א' בקובץ) עבר להיות הורה ב' באפליקציה
    parentBName: "אבי לוי",
...
test("כשלהורה ב' אין טלפון — הורה א' נשאר הראשי (לא מרוקנים את המספר)", () => {
  const grid = [
    ...
    [
      "רון", "כהן", "", "ז", "", "", "", "", "",
      "משה", "כהן", "050-1111111", "moshe@example.com", // הורה א' עם טלפון
      "רבקה", "כהן", "", "", // הורה ב' בלי טלפון
      "כן",
    ],
  ];
  const rows = parseStudentGrid(grid);
  ...
  expect(rows[0]).toMatchObject({
    parentName: "משה כהן",
    parentPhoneNumber: "050-1111111",
    parentBName: "רבקה כהן",
    parentBPhone: "",
  });
});
